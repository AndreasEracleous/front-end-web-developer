		...
		pages: [
			{
				blog: {
					title: 'Blog',
					subTitle: 'Practical tips for responsive web design.',
				},
			},
		],
	},
	plugins: [
		{
			...
			options: {
				test: /\.js$|\.jsx$/,
				exclude: /(node_modules|.cache|public|_this_is_virtual_fs_path_)/, // gatsby-plugin-eslint gives warnings on _this_is_virtual_fs_path_
				options: {
					emitWarning: true,
					failOnError: false,
				},
		...
		},
		{
			...
			options: {
				...
				...fs.readdirSync(path.join(__dirname, 'src')).reduce(
					(acc, dir) => ({
						...acc,
		...
		},
		{
			...
			options: {
				rule: {
					include: `${__dirname}/src/assets/img/svg/`,
				},
			},
		},
		{
		...
		},
		{
			...
			options: {
				component: require.resolve(
					`${__dirname}/src/components/layouts/index.jsx`,
				),
			},
		},
		`gatsby-plugin-sass`,
